...
alias yt3="yt-dlp -x --audio-format mp3 --audio-quality 0 --embed-thumbnail --embed-metadata --sponsorblock-remove all --progress --parse-metadata 'title:(?P<title>[^-]+) - (?P<artist>[^|]+)' --replace-in-metadata title:.*official.*music.*video.*:: -o '/storage/emulated/0/Music/%(artist,Unknown)s - %(title)s.%(ext)s'"
