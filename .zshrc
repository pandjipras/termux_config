...
alias yt3="yt-dlp -x --audio-format mp3 --audio-quality 0 --embed-thumbnail --embed-metadata --sponsorblock-remove all --progress --parse-metadata 'title:(?P<artist>.+) - (?P<title>.+)' --replace-in-metadata '^(.*)$' '\u\1' -o '/storage/emulated/0/Music/%(artist|titlecase)s - %(title|titlecase)s.%(ext)s'"
