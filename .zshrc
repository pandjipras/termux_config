...
alias yt4="yt-dlp -f 'bestvideo+bestaudio' --merge-output-format mp4 -o '/storage/emulated/0/Download/Ytdlp/%(title)s.%(ext)s' && echo 'Downloading video...'; \
            yt-dlp -f 'bestvideo+bestaudio' --merge-output-format mp4 -o '/storage/emulated/0/Download/Ytdlp/%(title)s.%(ext)s' && \
            echo 'Processing video with ffmpeg...'; \
            OUTPUT_FILE='/storage/emulated/0/Download/Ytdlp/%(title)s.mp4'; \
            nohup ffmpeg -nostdin -i \"$OUTPUT_FILE\" -c:v libx264 -c:a aac -strict experimental -y > /dev/null 2>&1 && echo 'Processing done!'; \
